...
action htwAgentHornet(WumpusWorldSensor currentSensor, bool shouldReset)
{
        // Declare all static variables.
        static bool hasGold;
        static bool killedWumpus;
        ...
        if(currentSensor.perceiveScream() || killedWumpus)
        {
            //then the wumpus is for certain dead, mark all rooms wumpus-free.
            for(int i = 0; i < 4; i++)
            {
                    for(int j = 0; j < 4; j++)
                    {
            ...
            //this is so that if I kill the wumpus but still smell the stentch,
            //I'll stil know that he's dead.
        }
                ...
                if(whereX - 1 >= 0)
                {
                        ...
                        if(Cave[whereX-1][whereY].visitedBefore == false)
                        {
                                if(currentSensor.perceiveStench())
                                {
                                    ...
                                    if(!shotArrow && Cave[whereX-1][whereY].Wumpus > 2)
                                    {
                ...
                if(whereY - 1 >= 0)
                {
                        if(Cave[whereX][whereY-1].visitedBefore == false)
                        {
                            if(currentSensor.perceiveStench())
                            {
                                ...
                                if(!shotArrow && Cave[whereX][whereY-1].Wumpus > 2)
                                {
                                    shotArrow = true;
