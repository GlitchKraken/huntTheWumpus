...
int main()
{
   ...
   cout << "\n"
        << "Overall standings:           average    times    times    times    times    times    rooms   arrows    wumpi\n"
        << "                              points     best     rich  gave up   killed  starved explored     shot   killed\n"
        << fixed;
   for (i = 0; i < numAgents; i += 1)
   {
      cout << setw(24) << left << agentStr[order[i]]
           ...
           << " " << setw(8) << right << roomsExplored[order[i]]
